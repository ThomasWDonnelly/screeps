let listOfRoles = ['builder', 'claimer', 'harvester', 'repairer', 'trucker', 'upgrader', 'wallRepairer'];
...
StructureSpawn.prototype.spawnCreepsIfNecessary =
    function () {
        ...
        // if no harvesters are left AND either no miners or no trucks are left
        //  create a backup creep
        if (numberOfCreeps['harvester'] == 0 && numberOfCreeps['trucker'] == 0) {
            // if there are still miners or enough energy in Storage left
            if (numberOfCreeps['miner'] > 0 ||
                (room.storage != undefined && room.storage.store[RESOURCE_ENERGY] >= 150 + 550)) {
                // create a trucker
                name = this.createtrucker(150);
            }
            // if there is no miner and not enough energy left in Storage
            else {
...
StructureSpawn.prototype.createtrucker =
    function (energy) {
        ...
        // create creep with the created body and the role 'trucker'
        return this.spawnCreep(body, 'trucker_' + Game.time, {memory: { role: 'trucker', working: false }});
    };
